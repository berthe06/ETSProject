...
def main():
    try:
        ...
        # Chargement des fichiers structurés issus de l'ingestion
        train_data = pd.read_csv('/content/drive/MyDrive/ProjetEts/HDFS_results/HDFS_train.log_structured_blk.csv')
        test_data = pd.read_csv('/content/drive/MyDrive/ProjetEts/HDFS_results/HDFS_test.log_structured_blk.csv')
        logger.debug("📥 Données chargées avec succès depuis Google Drive")

        # Fonction de prétraitement : nettoyage simple et transformation du label
        def preprocess(df):
        ...
        output_dir = '/content/drive/MyDrive/ProjetEts/HDFS_results/preprocessed'
        os.makedirs(output_dir, exist_ok=True)
